...
async function searchProjects({ keywords, timeLeft, hiredMin, proposalsMax, categories }) {
  return request('/api/search', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ keywords, timeLeft, hiredMin, proposalsMax, categories }),
  }, 300_000)
}
...
async function submitRespond({ url, cp_text, duration }) {
  return request('/api/respond', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, cp_text, duration }),
  }, 60_000)
}
...
module.exports = { parseProject, searchProjects, getProjects, generateCp, submitRespond, searchWorkzilla, submitWorkzillaRespond }
