...
async function createRespondJob({ url, cp_text, duration, title, description }) {
  return request('/api/respond/jobs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, cp_text, duration, title, description }),
  }, 90_000)
...
async function getRespondJobStatus(jobId, since = 0) {
  return request(`/api/respond/jobs/${encodeURIComponent(jobId)}?since=${Number(since) || 0}`, {}, 15_000)
}
