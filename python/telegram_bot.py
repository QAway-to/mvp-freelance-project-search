...
class TelegramNotifier:
    ...
    async def send_project_notification(self, project: Dict[str, Any]):
        ...
        try:
            ...
            # Format message
            urgency = project.get('urgency', 'Не указано')
            message = f"🔥 <b>СРОЧНЫЙ ПРОЕКТ! ({urgency})</b>\n\n"

            message += f"📋 <b>Название:</b> {project.get('title', 'N/A')}\n"
            message += f"💰 <b>Бюджет:</b> {project.get('budget', 'N/A')}\n"
            message += f"⏳ <b>Осталось:</b> {urgency}\n"
            message += f"📊 <b>Релевантность:</b> {score:.2f}/1.0\n\n"

            message += f"📝 <b>Описание:</b>\n{project.get('description', 'N/A')[:500]}...\n\n"

            message += f"🔗 <b>Ссылка:</b> {project.get('url', 'N/A')}\n\n"
            
            message += f"✨ <b>Команда для Gemini:</b>\n"
            message += f"<code>Создай коммерческое предложение на заказ {project.get('url', 'N/A')}</code>"
