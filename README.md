...
- **Demo режим**: Рекомендуется для тестирования - не требует браузера, работает быстро
- **Full режим**: Требует headless браузер - может потребоваться больше ресурсов
- **Chrome Driver**: Автоматически загружается через webdriver-manager
- **Таймауты**: Render бесплатный план имеет ограничения на время выполнения запросов
